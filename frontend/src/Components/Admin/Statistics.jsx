...
const Statistics = () => {
  ...
  if (loading) {
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {[1, 2, 3, 4].map(i => (
          <div key={i} className="bg-gray-100 rounded-2xl p-5 animate-pulse h-32" />
        ))}
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      <StatCard title="Revenue" value={`₹${(stats.revenue?.current || 0).toLocaleString()}`} change={stats.revenue?.change || 0} gradient="bg-gradient-to-br from-violet-600 via-violet-700 to-indigo-800"
