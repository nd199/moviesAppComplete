...
const AdminHeader = () => {
  ...
  return (
    <header className="bg-surface-900 border-b border-surface-700 px-6 py-4 flex items-center justify-between flex-shrink-0">
      <div>
        <h1 className="text-xl font-bold text-white tracking-tight">{getPageTitle()}</h1>
        <p className="text-sm text-surface-500 mt-0.5">{getPageBreadcrumb()}</p>
      </div>

      <div className="flex items-center gap-3">
        {/* User badge */}
        <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-xl bg-surface-800 border border-surface-700">
        ...
        <button className="relative p-2.5 text-surface-500 hover:text-white hover:bg-surface-800 rounded-xl transition-all duration-200">
          <IoNotificationsOutline className="h-5 w-5" />
          <span className="absolute top-2 right-2 h-2 w-2 bg-red-500 rounded-full ring-2 ring-surface-900"></span>
        </button>

        <button onClick={handleLogout}
          className="flex items-center gap-2 px-3.5 py-2 text-sm font-medium text-surface-500 hover:text-red-400 hover:bg-red-500/10 rounded-xl transition-all duration-200">
