...
const LikeButton = ({ tmdbId, mediaType, title, size = 'medium', showLabel = true, showCount = false, className = '', onSuccess, onError }) => {
  ...
  useEffect(() => {
    ...
    if (isAuth) {
      likesAPI.checkLiked(tmdbId, mediaType)
        .then(r => setLiked(r.data.liked))
        .catch(() => {});
    }
    if (showCount) {
      likesAPI.getTotalLikes(tmdbId, mediaType)
        .then(r => setCount(r.data.totalLikes))
        .catch(() => {});
    }
  ...
  const toggle = async (e) => {
    ...
    try {
      if (liked) {
        await likesAPI.unlike(tmdbId, mediaType);
        setLiked(false);
        setCount(c => (c === null ? c : Math.max(0, c - 1)));
        onSuccess?.('Removed');
      } else {
        await likesAPI.like({ tmdbId, title: title || '', mediaType, likeStatus: 'LIKE' });
        setLiked(true);
