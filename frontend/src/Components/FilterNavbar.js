...
import { fetchMovieGenres, fetchTvGenres } from '../Network/ApiCalls';

const FilterNavbar = ({ sortBy, setSortBy, searchQuery, setSearchQuery, genre, setGenre, year, setYear, rating, setRating, mediaType = 'movie' }) => {
  const [filtersOpen, setFiltersOpen] = useState(false);
  const [genres, setGenres] = useState([]);

  useEffect(() => {
    const load = async () => {
      const data = mediaType === 'tv' ? await fetchTvGenres() : await fetchMovieGenres();
      setGenres(data);
    };
    load();
  }, [mediaType]);
