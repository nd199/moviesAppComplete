...
const ShowList = () => {
    ...
    }, [currentPage, filteredShows, itemsPerPage]);
    ...
    useEffect(() => {
        debouncedSearch(searchTerm, shows);
        setCurrentPage(1);
        return () => debouncedSearch.cancel();
    }, [searchTerm, shows, debouncedSearch]);
    ...
    const stats = useMemo(() => {
            ...
            const avgRating = shows.length > 0 ?
                (shows.reduce((sum, s) => sum + (s.rating || 0), 0) / shows.length).toFixed(
                    1) : '0.0';
            const categories = [...new Set(
                shows.map(s => s.category).filter(Boolean
                ))].length;
            return {totalShows, avgRating, categories};
        }, [shows])
    ;
    ...
    return (
        <div className="space-y-4 sm:space-y-6">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                {[
                    {label: 'Total Shows', value: stats.totalShows, icon: Tv, color: 'from-brand-600 to-brand-700'},
                    {label: 'Categories', value: stats.categories, icon: Star, color: 'from-emerald-500 to-teal-500'},
                    {
                        label: 'Avg. Rating',
                    ...
                    <table className="w-full text-sm">
                        ...
                        {loading ? (
                                ...
                                <tr key={show.show_id} className="hover:bg-surface-800/50 transition-colors">
                                    ...
                                    <td className="px-4 py-3">
                                        <div className="flex items-center gap-1">
                                            ...
                                            <span
                                                className="text-white font-medium">{show.rating != null ? (Math.floor(show.rating * 100) / 100).toFixed(2) : '—'}</span>
                                        </div>
                                    </td>
                          ...
                          {show.genre}
                                    ...
                                    <td className="px-4 py-3">
                      <span
                          className="px-2 py-1 text-xs bg-emerald-500/15 text-emerald-400 border border-emerald-500/20 rounded-lg">
                ...
                {loading ? (
                    ...
                    <div className="bg-surface-900 border border-surface-700 rounded-2xl p-6 text-center">
                        <div className="flex flex-col items-center gap-3">
                            ...
                            <p className="text-surface-500 text-sm">No shows found</p>
                            <Link to="/admin/shows/new" className="text-brand-400 text-sm hover:text-brand-300">Add your
                                first show</Link>
                        </div>
                    </div>
                ) : (
                    paginatedShows.map((show) => (
                        <div key={show.show_id} className="bg-surface-900 border border-surface-700 rounded-2xl p-4">
                            ...
                            <div className="flex flex-wrap gap-2 text-xs">
                                {show.genre && (
                                    <span
                                        className="px-2 py-1 bg-brand-500/15 text-brand-400 border border-brand-500/20 rounded-lg">{show.genre}</span>
                                )}
                                {show.category && (
                                    <span
                                        className="px-2 py-1 bg-emerald-500/15 text-emerald-400 border border-emerald-500/20 rounded-lg">{show.category}</span>
                                )}
                                {show.runtime && (
                                    <span
                                        className="px-2 py-1 bg-surface-800 text-surface-500 rounded-lg">{show.runtime}m</span>
                                )}
                            </div>
                        </div>
                    ))
            ...
            {totalPages > 1 && (
                <div className="flex items-center justify-between pt-4">
                    <p className="text-sm text-surface-500">
                        Showing {(currentPage - 1) * itemsPerPage + 1} to {Math.min(currentPage * itemsPerPage, filteredShows.length)} of {filteredShows.length} shows
                    </p>
