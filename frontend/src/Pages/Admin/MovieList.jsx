  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        {[
          { label: 'Total Movies', value: totalMovies, icon: Film, color: 'from-brand-600 to-brand-700' },
          { label: 'Avg. Rating', value: avgRating, icon: Star, color: 'from-amber-500 to-orange-500', prefix: '★' },
          { label: 'Genres', value: genres, icon: Calendar, color: 'from-emerald-500 to-teal-500' },
          { label: 'Filtered', value: filteredMovies.length, icon: Clock, color: 'from-accent-600 to-accent-700' },
        ].map((stat) => (
            ...
            <div key={movie.id} className="bg-surface-900 border border-surface-700 rounded-2xl p-4">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-3 min-w-0">
                  {movie.poster && (
                    <img className="w-10 h-10 rounded-lg object-cover border border-surface-700 flex-shrink-0" src={movie.poster} alt="Poster" />
                  )}
              ...
              <div className="flex flex-wrap gap-2 text-xs">
                {movie.genre && (
                  <span className="px-2 py-1 bg-brand-500/15 text-brand-400 border border-brand-500/20 rounded-lg">{movie.genre}</span>
                )}
                {movie.runtime && (
                  <span className="px-2 py-1 bg-surface-800 text-surface-500 rounded-lg">{movie.runtime}m</span>
                )}
                {movie.description && (
