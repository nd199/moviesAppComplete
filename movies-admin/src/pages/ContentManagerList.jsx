...
const ContentManagerList = () => {
  ...
  const fetchContentManagers = async () => {
    setLoading(true);
    try {
      const response = await contentManagerApi.getAllContentManagers();
      
      // Fetch analytics for each content manager
      const contentManagersWithAnalytics = await Promise.all(
        response.map(async (cm) => {
          try {
            const analytics = await contentManagerApi.getContentManagerAnalytics(cm.id);
            return {
              ...cm,
              moviesManaged: analytics.movieCount || 0,
              showsManaged: analytics.showCount || 0
            };
          } catch (error) {
            return {
              ...cm,
      ...
      setContentManagers(contentManagersWithAnalytics);
    } catch (error) {
  ...
  const deleteContentManager = async (id) => {
    if (window.confirm('Are you sure you want to delete this content manager?')) {
      try {
        await contentManagerApi.deleteContentManager(id);
                  ...
                  <tr key={cm.id} className="hover:bg-slate-700/30">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          onClick={() => deleteContentManager(cm.id)}
